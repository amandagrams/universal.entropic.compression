...
namespace universal.entropic.compression.Menu
{
    class DoCrc8 : Page
    {
        ...
        public override void Display()
        {
            ...
            Output.WriteLine("Apply crc8 and hamming to sum encoded with Delta");
            //Read a file
            byteList = File.ReadAllBytes(Utils.Utils.FilesEncoded.DeltaEncodeSum).ToList();
            newBytesFile = byteList.Take(2).ToList();
            //Applay crc-8 to 2 first bytes
            newBytesFile.Add(crc8.CRC_8(byteList.Take(2).ToArray()));
            foreach (var item in byteList.Skip(2).ToList())
            ...
            File.WriteAllBytes(Utils.Utils.FilesEncoded.Crc8HammingEncodeDeltaEncodeSum, newBytesFile.ToArray());
            Output.WriteLine(System.ConsoleColor.Green, "View the file encoded in: " + Utils.Utils.FilesEncoded.Crc8HammingEncodeDeltaEncodeSum.ToString());
            newBytesFile.Clear();
            byteList.Clear();
            Output.WriteLine("");
            Output.WriteLine("");
            ...
            Output.WriteLine("Apply crc8 and hamming to Alice29.txt encoded with Delta");
            //Read a file
            byteList = File.ReadAllBytes(Utils.Utils.FilesEncoded.DeltaEncodeAlice).ToList();
            newBytesFile = byteList.Take(2).ToList();
            //Applay crc-8 to 2 first bytes
            newBytesFile.Add(crc8.CRC_8(byteList.Take(2).ToArray()));
            foreach (var item in byteList.Skip(2).ToList())
            ...
            File.WriteAllBytes(Utils.Utils.FilesEncoded.Crc8HammingEncodeDeltaEncodeAlice, newBytesFile.ToArray());
            Output.WriteLine(System.ConsoleColor.Green, "View the file encoded in: " + Utils.Utils.FilesEncoded.Crc8HammingEncodeDeltaEncodeAlice.ToString());
            newBytesFile.Clear();
            byteList.Clear();
            Output.WriteLine("");
            Output.WriteLine("");
